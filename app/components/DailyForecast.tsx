    ...
    <div className="mt-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3 md:gap-4">
        ...
        {days.map((item: any, index: number) => (
          <div
            key={index}
            className={`bg-brand-card border border-brand-border rounded-3xl p-4 md:p-5 flex flex-col items-center gap-3 md:gap-4 min-h-[160px] ${
              isDataLoading
                ? "opacity-40 animate-pulse"
                : "hover:border-brand-blue group cursor-pointer"
            }`}
          >
            {isDataLoading ? (
              <div className="w-full h-full bg-brand-muted/10 rounded-2xl" />
            ) : (
              <>
